/**
 * ============================================================
 * ADMIN PORT DATASET – Main Core JS
 * resources/js/admin/ports/ports.js
 * ============================================================
 */
...
const PortsCore = (() => {
    ...
    function initSkeletonLoader() {
        const skeleton = document.getElementById('ports-skeleton-container');
    ...
    function refreshData() {
        ...
        if (skeleton && content) {
            ...
            setTimeout(() => {
                skeleton.style.display = 'none';
                content.style.display = 'block';
                showToast('Dataset pelabuhan berhasil diperbarui dari data satelit!');
            }, 800);
        }
    }
    ...
    function showToast(message) {
        ...
        if (message.includes('berhasil') || message.includes('impor') || message.includes('Import') || message.includes('ditambahkan') || message.includes('diperbarui') || message.includes('dihapus')) {
            const successAlertContainer = document.getElementById('ports-success-alert-container');
            const successAlertText = document.getElementById('ports-success-alert-text');
            if (successAlertContainer && successAlertText) {
                successAlertText.textContent = message;
                successAlertContainer.style.display = 'block';
                
                setTimeout(() => {
                    successAlertContainer.style.display = 'none';
                }, 5000);
            }
        }
    }
    ...
    function simulateError() {
        const content = document.getElementById('ports-main-content');
        const errorAlert = document.getElementById('ports-error-container');
